...
abund_table<-read.csv("alps_vOTUs.csv",row.names=1,check.names=FALSE)
#Transpose the data to have sample names on rows
abund_table<-t(abund_table)

meta_table2<-read.csv("ENV.csv",row.names=1,check.names=T)
meta_table <- meta_table2[,-1]
...
abund_table.adonis <- abund_table.adonis
abund_table.adonis
#Call:
